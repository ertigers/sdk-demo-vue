const WebGLDisplayer = function(canvas) {
    this.canvas = canvas;
    this.gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
    this._init();
}
WebGLDisplayer.prototype._init = function() {
    let gl = this.gl;
    if (!gl) {
...
WebGLDisplayer.prototype._initVertexBuffers = function(program) {
    let gl = this.gl;
 ...
 * @return {glShader} 着色器。
 */
WebGLDisplayer.prototype._compileShader = function(shaderSource, shaderType) {
    // 创建着色器程序
 ...
 * @return {glProgram} 程序
 */
WebGLDisplayer.prototype._createProgram = function(vertexShader, fragmentShader) {
    const gl = this.gl;
 ...
 * 设置纹理
 */
WebGLDisplayer.prototype._createTexture = function(filter = this.gl.LINEAR) {
    let gl = this.gl;
 ...
 */
WebGLDisplayer.prototype.renderImg = function(width, height, data) {
    let gl = this.gl;
 ...
 */
WebGLDisplayer.prototype.setSize = function(width, height, maxWidth) {
    let canvasWidth = Math.min(maxWidth, width);
    this.canvas.width = canvasWidth;
    this.canvas.height = canvasWidth * height / width;
}

WebGLDisplayer.prototype.destroy = function() {
    const {
        gl
    } = this;
