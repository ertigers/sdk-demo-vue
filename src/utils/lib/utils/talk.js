...
(function(window) {
    ...
    function dtcWebsocket(websocket_url, IP, Port, Token, queryToken, name) { // 连接websocket
        let url = websocket_url + '?token=' + queryToken + '&IP=' + IP + '&Port=' + Port + '&Token=' + Token + '&dtc=true'
        console.log(url);
    ...
    window.QXstartCall = function(q2http_url, websocket_url, token, puid, idx, name, callback) { //开启对讲,每开启一个对讲就开启一个websocket,关闭对讲就要关闭websocket
        if (callback) {
            cb[name] = callback;
        }
        var params = {
        ...
        QxRequest("post", q2http_url + "audio/startCall?token=" + token, params).then((res)=>{
            if (res.hasOwnProperty('errcode')) {
                var message = {
                    type: "startCall_error",
    ...
    window.QXstartTalk = function(q2http_url, websocket_url, token, puid, idx, name, callback) { //开启对讲,每开启一个对讲就开启一个websocket,关闭对讲就要关闭websocket
        if (callback) {
            cb[name] = callback;
        }
        ...
        QxRequest("post", q2http_url + "audio/startTalk?token=" + token, params).then((res)=>{
            console.log(res);
            if (res.hasOwnProperty('errcode')) {
                var message = {
    ...
    window.QXstopCall = function(name) {
        console.log(name);
        if (recorder[name]) {
    ...
    window.QXstopTalk = function(q2http_url, puid, idx, name) {
        return new Promise((resolve)=>{
            let params = {
                puid: puid,
                idx: idx
            }
            QxRequest("post", q2http_url + "audio/stopTalk", params).then((res)=>{
                let message = ''
                if (res.hasOwnProperty('errcode')) {
                    message = {
                        type: "startTalk_error",
